...
function stationAngle(station: RetroRadioStation, index: number, count: number) {
...
export function RetroRadio({
  ...
  const initialTuneAngle = stations[safeInitialIndex]
    ? stationAngle(stations[safeInitialIndex], safeInitialIndex, stations.length)
    : 0
  ...
  const nearestStation = useCallback(
    (angle: number) => {
      ...
      stations.forEach((item, index) => {
        const distance = Math.abs(stationAngle(item, index, stations.length) - angle)
  ...
  const selectStation = useCallback(
    (nextIndex: number) => {
      ...
      const nextAngle = stationAngle(stations[normalized], normalized, stations.length)
  ...
  const updateDial = (
    ...
    current.dialAngle = clamp(current.dialAngle + delta * KNOB_DAMPING, minimum, maximum)
  ...
  const finishTune = (event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    if (moved) selectStation(nearestStation(tuneGesture.current.dialAngle).index)
  ...
  const cancelDial = (event: ReactPointerEvent<HTMLButtonElement>, gesture: typeof tuneGesture) => {
  ...
  const handleTuneKey = (event: KeyboardEvent<HTMLButtonElement>) => {
    ...
    event.preventDefault()
    selectStation(stationIndexRef.current + (event.key === 'ArrowRight' ? 1 : -1))
  }
  ...
  const beginAntennaDrag = (event: ReactPointerEvent<SVGCircleElement>) => {
    ...
    const move = (moveEvent: PointerEvent) => {
      ...
      setAntenna({
        ...
        length: clamp(Math.hypot(dx, dy), ANTENNA_MIN_LENGTH, ANTENNA_MAX_LENGTH),
      })
    }
  ...
  useEffect(() => {
    ...
    const nextIndex = Math.round(clamp(stationIndexRef.current, 0, stations.length - 1))
    const nextAngle = stationAngle(stations[nextIndex], nextIndex, stations.length)
  ...
  if (!displayStation) {
    return <section className="retro-radio retro-radio--empty">No stations</section>
  }
  ...
  const antennaTicks = Array.from({ length: antennaSegmentCount - 1 }, (_, index) => {
      ...
      <div className="retro-radio__stage">
        <div className={`retro-radio__body ${switching ? 'is-switching' : ''}`}>
          <div className="retro-radio__tilt">
            ...
            >
              <defs>
                <pattern id={patternId} patternUnits="userSpaceOnUse" width="660" height="440">
                  <image
                    href={woodTexture}
                    x="-40"
                    y="-60"
                ...
                <linearGradient id={antennaGradientId} x1="0" y1="0" x2="1" y2="0">
                ...
                <linearGradient id={clothGradientId} x1="0" y1="0" x2="0" y2="1">
                ...
                <filter id={clothTextureId} x="0" y="0" width="100%" height="100%">
                  <feTurbulence
                    type="turbulence"
              ...
              <g className="retro-radio__antenna">
                ...
                {antennaTicks.map((tick, index) => (
                  <line key={index} {...tick} stroke="#2b1b0c" strokeWidth=".9" opacity=".72" />
                ))}
                <circle
                  cx={ANTENNA_BASE.x}
                  cy={ANTENNA_BASE.y}
                  ...
                  strokeWidth="1.2"
                />
                <circle cx={ANTENNA_BASE.x} cy={ANTENNA_BASE.y} r="2.4" fill="#c9a24b" />
                <g
              ...
              <g filter={`url(#${shadowId})`}>
                <rect x="20" y="70" width="620" height="300" rx="26" fill={`url(#${patternId})`} />
              </g>
              <g clipPath={`url(#${cabinetClipId})`}>
                <rect x="20" y="70" width="620" height="300" fill={`url(#${woodLightId})`} />
                <rect x="20" y="70" width="620" height="300" fill={`url(#${vignetteId})`} />
              </g>
              <rect
                x="20"
                y="70"
              ...
              <rect
                ...
                opacity=".22"
              />
              <rect x="42" y="92" width="258" height="218" rx="14" fill="#0e0904" />
              <rect
                x="46"
                y="96"
                width="250"
              ...
              <g clipPath={`url(#${speakerClipId})`}>
                <rect x="46" y="96" width="250" height="210" fill={`url(#${clothGradientId})`} />
                <rect
                  x="46"
                  y="96"
                  width="250"
              ...
              {slots.map((y) => (
                <g key={y}>
                  ...
                  />
                  <rect x="56" y={y} width="230" height="2" rx="1" fill="#c69a5a" opacity=".32" />
                </g>
              ))}
              <rect
                x="326"
                ...
                strokeWidth="2"
              />
              <rect x="332" y="96" width="280" height="128" rx="12" fill="#050a04" />
              <rect
                x="326"
                y="90"
                width="292"
              ...
              <ellipse cx="414" cy="334" rx="40" ry="12" fill="#0d0804" opacity=".6" />
              <ellipse cx="530" cy="334" rx="40" ry="12" fill="#0d0804" opacity=".6" />
              <rect x="70" y="366" width="72" height="16" rx="6" fill="#160d06" />
              <rect x="520" y="366" width="72" height="16" rx="6" fill="#160d06" />
            </svg>
            ...
            >
              <div className="retro-radio__crt-view retro-radio__crt-mode">
                <span className="retro-radio__glyph">{displayStation.glyph}</span>
            ...
            <button
              ...
              onPointerDown={(event) => beginDial(event, tuneGesture, tuneAngleRef.current)}
            ...
            <button
              ...
              onPointerDown={(event) => beginDial(event, volumeGesture, volumeAngle)}
