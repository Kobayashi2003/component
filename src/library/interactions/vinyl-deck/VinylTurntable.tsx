...
export function VinylTurntable({
  ...
  const volumeDialRef = useRef<SVGGElement>(null)
  ...
  // Shared keyboard activation -------------------------------------------------
  useEffect(() => {
    const dial = volumeDialRef.current
    if (!dial) return
    // SVG groups do not consistently honor touch-action across touch browsers.
    // A native non-passive listener prevents panning only for this control.
    const blockScroll = (event: TouchEvent) => {
      if (event.cancelable) event.preventDefault()
    }
    dial.addEventListener('touchmove', blockScroll, { passive: false })
    return () => dial.removeEventListener('touchmove', blockScroll)
  }, [])
  ...
  const updateVolumeFromPointer = (event: PointerEvent<SVGGElement>) => {
    if (!volumeDragRef.current || volumePointerRef.current !== event.pointerId) return
    event.preventDefault()
    ...
    const nextAngle = Math.max(
      -130,
      Math.min(130, volumeGestureRef.current.dialAngle + delta * 0.38),
    )
  ...
  const handleVolumePointerDown = (event: PointerEvent<SVGGElement>) => {
    if (!event.isPrimary || event.button !== 0 || volumePointerRef.current !== null) return
    event.preventDefault()
    event.stopPropagation()
  ...
  const finishVolumeDrag = (event: PointerEvent<SVGGElement>) => {
    if (volumePointerRef.current !== event.pointerId) return
    event.preventDefault()
    event.stopPropagation()
    volumeDragRef.current = false
    volumePointerRef.current = null
    if (event.type !== 'pointerup') volumeGestureRef.current.moved = true
    if (event.currentTarget.hasPointerCapture(event.pointerId))
      event.currentTarget.releasePointerCapture(event.pointerId)
  }
  ...
  return (
    ...
    >
      ...
      <g className="vinyl-deck__blueprint" aria-hidden="true">
        ...
        <path className="vinyl-deck__tonearm-arm" pathLength="1" d="M680 111L585 292L548 337" />
      ...
      {/* 03. Hardware pass: the finished, interactive machine. */}
      <g className="vinyl-deck__hardware">
        ...
        >
          ...
          <text
            className="vinyl-deck__side-mode-label"
            textAnchor="middle"
            dominantBaseline="middle"
          >
            {'AUTO'.split('').map((letter, letterIndex) => (
              <tspan key={letter} x="-3" y={133 + letterIndex * 11}>
                {letter}
              </tspan>
            ))}
          </text>
        ...
        >
          ...
          <text
            ...
            dominantBaseline="middle"
          >
            {'SHUFFLE'.split('').map((letter, letterIndex) => (
              <tspan key={`${letter}-${letterIndex}`} x="-3" y={209 + letterIndex * 11}>
                {letter}
              </tspan>
            ))}
          </text>
        ...
        <g className="vinyl-deck__assembly" strokeLinejoin="round">
          <path
            className="vinyl-deck__chassis"
            d="M63 6H728L812 82V460L744 540H59L8 484V72L63 6Z"
            fill={`url(#${bevelId})`}
            stroke="#111416"
            strokeWidth="3"
          />
          <path
            className="vinyl-deck__panel"
            d="M82 32H708L780 96V462L736 526H70L38 472V91L82 32Z"
            fill={`url(#${shellId})`}
            stroke="#e4e5e5"
            strokeWidth="2"
          />
          ...
          <path
            className="vinyl-deck__draw draw-01"
            pathLength="1"
            d="M63 6H728L812 82V460L744 540H59L8 484V72L63 6Z"
            stroke="#15181b"
            strokeWidth="4"
          />
          <path
            className="vinyl-deck__draw draw-02"
            pathLength="1"
            d="M82 32H708L780 96V462L736 526H70L38 472V91L82 32Z"
            stroke="#85898c"
            strokeWidth="1.5"
          />
          <path
            className="vinyl-deck__seam vinyl-deck__draw draw-03"
            pathLength="1"
            d="M63 63H286L310 39"
          />
          {[
            ...
            <g
              className="vinyl-deck__screw"
              key={`${screw.x}-${screw.y}`}
              transform={`translate(${screw.x} ${screw.y})`}
            >
              <circle r="7" fill="#4d5255" />
              <circle r="3" fill="#171a1d" />
              <path d="M-2.5-2.5L2.5 2.5" stroke="#aeb1b2" strokeWidth="1" />
            </g>
          ))}
        </g>

        {/* Transport: previous / next */}
        <g
        ...
        >
          <path
            className="vinyl-deck__control-outline vinyl-deck__key-face"
            d="M72 108H183L200 125V169L184 184H72L59 171V122Z"
            fill={`url(#${keyId})`}
            stroke="#111416"
            strokeWidth="3"
          />
          <path
            d="M183 108L200 125V169L184 184H172L185 169V124L169 108Z"
            fill="#555a5d"
            opacity=".7"
          />
          <path
            className="vinyl-deck__key-accent"
            d="M72 108H183"
            stroke="var(--focus-accent)"
            strokeWidth="2"
          />
          <text x="126" y="151" textAnchor="middle">
            PREV
          </text>
          <text className="vinyl-deck__key-code" x="74" y="173">
            ◀ 01
          </text>
          <path
            className="vinyl-deck__hit-area"
            d="M72 108H183L200 125V169L184 184H72L59 171V122Z"
            fill="transparent"
          />
        </g>
        ...
        >
          <path
            className="vinyl-deck__control-outline vinyl-deck__key-face"
            d="M72 203H183L200 220V264L184 279H72L59 266V217Z"
            fill={`url(#${keyId})`}
            stroke="#111416"
            strokeWidth="3"
          />
          <path
            d="M183 203L200 220V264L184 279H172L185 264V219L169 203Z"
            fill="#555a5d"
            opacity=".7"
          />
          <path
            ...
            strokeWidth="2"
          />
          <text x="126" y="246" textAnchor="middle">
            NEXT
          </text>
          <text className="vinyl-deck__key-code" x="74" y="268">
            ▶ 02
          </text>
          <path
            className="vinyl-deck__hit-area"
        ...
        {/* Left utility module: output label + volume control */}
        <path
        ...
        <g className="vinyl-deck__pitch" stroke="currentColor" transform="translate(-10 0)">
          ...
          <text x="184" y="327" textAnchor="end">
            03
          </text>
          <path d="M68 338H184" stroke="#85898c" strokeWidth="1" />
        </g>

        {/* Platter: drag target, rotor, record artwork and spindle */}
        <g
        ...
        >
          ...
          />
          <circle
            className="vinyl-deck__draw vinyl-deck__platter-outline draw-09"
            pathLength="1"
            cx="414"
            cy="274"
            r="205"
          ...
          <g ref={platterRotorRef} className="vinyl-deck__rotor">
            <g className="vinyl-deck__rings vinyl-deck__rings--outer">
              ...
              <circle
                className="vinyl-deck__segmented-ring ring-01"
                pathLength="1"
                cx="414"
                cy="274"
                r="181"
                stroke="#15191c"
                strokeWidth="14"
                strokeDasharray=".19 .026 .055 .018"
                transform="rotate(-18 414 274)"
              />
              <circle
                className="vinyl-deck__segmented-ring ring-02"
                pathLength="1"
                cx="414"
                cy="274"
                r="165"
                stroke="#aeb1b2"
                strokeWidth="6"
                strokeDasharray=".11 .022 .034 .016"
                transform="rotate(24 414 274)"
              />
            </g>
            <g className="vinyl-deck__rings vinyl-deck__rings--inner">
              <circle
                className="vinyl-deck__segmented-ring ring-03"
                pathLength="1"
                cx="414"
                cy="274"
                r="153"
                stroke="#171a1d"
                strokeWidth="12"
                strokeDasharray=".075 .018 .16 .03"
                transform="rotate(-38 414 274)"
              />
              <circle
                className="vinyl-deck__segmented-ring ring-04"
                pathLength="1"
                cx="414"
                cy="274"
                r="141"
                stroke="#74787a"
                strokeWidth="4"
                strokeDasharray=".015 .009 .12 .018"
                transform="rotate(11 414 274)"
              />
              ...
              <g clipPath={`url(#${labelClipId})`}>
                {cover ? (
                  ...
                  <>
                    ...
                    <path
                      d="M294 324L464 154H534V224L364 394H294Z"
                      fill="var(--focus-secondary)"
                      opacity=".46"
                    />
                    <text className="vinyl-deck__cover-title" x="414" y="266" textAnchor="middle">
                      {item.title}
                    </text>
                    <text className="vinyl-deck__cover-author" x="414" y="288" textAnchor="middle">
                      {item.author}
                    </text>
                  </>
                )}
        ...
        >
          ...
          <text className="vinyl-deck__volume-value" x="184" y="360" textAnchor="end">
            {String(volume).padStart(3, '0')}
          </text>
          <circle
            className="vinyl-deck__dial-mount"
            cx="128"
            cy="426"
            r="43"
            fill="#b8bbbc"
          ...
          {Array.from({ length: 21 }, (_, tick) => {
            ...
            return (
              <g
                key={tick}
                className={`vinyl-deck__dial-tick ${chargedTick ? 'is-charged' : ''} ${selectedTick ? 'is-selected' : ''}`.trim()}
              >
                <line
                  x1={128 + Math.cos(angle) * 34}
                  y1={426 + Math.sin(angle) * 34}
                  x2={128 + Math.cos(angle) * (selectedTick ? 41 : majorTick ? 39 : 37)}
                  y2={426 + Math.sin(angle) * (selectedTick ? 41 : majorTick ? 39 : 37)}
                />
                {selectedTick && (
                  <circle cx={128 + Math.cos(angle) * 43} cy={426 + Math.sin(angle) * 43} r="2.5" />
                )}
              </g>
            )
          })}
          <g className="vinyl-deck__dial-knob">
            ...
            <path
              className="vinyl-deck__volume-pointer"
              d="M128 426V406"
              stroke="#f1f1ef"
              strokeWidth="4"
              strokeLinecap="round"
        ...
        {/* Right utility module: signal meter + play/pause control */}
        <path
        ...
        <g className="vinyl-deck__meter" transform="translate(10 0)">
          <path
            d="M646 318H739L746 325V380H646L640 374V324Z"
            fill="#15191c"
            stroke="#34393c"
            strokeWidth="3"
          />
          ...
          <text x="736" y="329" textAnchor="end">
            {item.bpm ?? '--'}
          </text>
          <text x="652" y="345">
            L
          ...
          {Array.from({ length: 13 }, (_, meterIndex) => (
            <line
              ...
              y1="368"
              x2={662 + meterIndex * 6}
              y2={368 - meterLevels[meterIndex] * 22}
              stroke="var(--focus-accent)"
              strokeWidth="3"
            />
          ))}
        ...
        >
          <path
            className="vinyl-deck__control-outline vinyl-deck__key-face"
            d="M646 402H739L746 409V463L739 470H646L640 464V408Z"
            fill={`url(#${keyId})`}
            stroke="#111416"
            strokeWidth="3"
          />
          <path
            className="vinyl-deck__key-inset"
            d="M650 408H735L740 413V459L735 464H650L646 460V412Z"
            fill="none"
            stroke="#676c6f"
            strokeWidth="1.5"
          />
          ...
          <circle
            ...
            cy="414"
            r="3.5"
            fill="var(--focus-accent)"
            className="vinyl-deck__status-light"
          />
          <text x="693" y="442" textAnchor="middle">
            {playing ? 'PAUSE' : 'PLAY'}
          </text>
          <path
            className="vinyl-deck__hit-area"
        ...
        {/* Tonearm: pivot housing + animated arm/cartridge assembly */}
        <g className="vinyl-deck__tonearm" transform="translate(10 8)">
          <path
            d="M640 53H720L740 73V149L720 169H640L620 149V73Z"
            fill="#222629"
            stroke="#111416"
            strokeWidth="3"
          />
          ...
          {[0, 90, 180, 270].map((angle) => (
            <circle
              key={angle}
              cx={680 + Math.cos((angle * Math.PI) / 180) * 34}
              cy={111 + Math.sin((angle * Math.PI) / 180) * 34}
              r="3.5"
              fill="var(--focus-accent)"
            />
          ))}
          <g className="vinyl-deck__tonearm-arm">
            ...
            <path
              className="vinyl-deck__cartridge"
              d="M570 307L553 338L532 326L551 294Z"
              fill="#272b2e"
              stroke="#101315"
              strokeWidth="3"
            />
