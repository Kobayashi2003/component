...
export class BrowserPublicationMediaRouter {
  ...
  constructor(private readonly onImage: (activation: ReaderImageActivation) => void) {}
  ...
  private attach(document: Document): () => void {
    const enhanced = Array.from(document.images).filter(image => {
      ...
      return !image.closest('a') && image.getAttribute('aria-hidden') !== 'true' && role !== 'presentation' && role !== 'none';
    });
    ...
    for (const image of enhanced) {
      ...
      if (!image.hasAttribute('aria-label')) image.setAttribute('aria-label', image.alt.trim() ? `View image: ${image.alt.trim()}` : 'View publication image');
    }

    const activate = (target: EventTarget | null): boolean => {
      const image = closestImage(target);
      if (!image || !image.hasAttribute(VIEWER_ATTRIBUTE) || image.closest('a')) return false;
      const src = image.currentSrc || image.src;
      if (!src) return false;
      const caption = image.closest('figure')?.querySelector('figcaption')?.textContent?.replace(/\s+/gu, ' ').trim();
      this.onImage({
        src,
        alt: image.alt.trim(),
        ...(caption ? { caption } : {}),
        ...(image.naturalWidth > 0 ? { intrinsicWidth: image.naturalWidth } : {}),
        ...(image.naturalHeight > 0 ? { intrinsicHeight: image.naturalHeight } : {}),
        trigger: image,
      });
      return true;
    };
    const onClick = (event: Event) => {
      ...
      if (click.button !== 0 || click.altKey || click.ctrlKey || click.metaKey || click.shiftKey || !activate(click.target)) return;
...
function closestImage(target: EventTarget | null): HTMLImageElement | null {
  ...
  return image?.localName.toLowerCase() === 'img' ? image as HTMLImageElement : null;
}
