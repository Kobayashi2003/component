import { loadEpub, type PublicationDiagnostic } from '../publication';
import { createCompatibilityReport, type CompatibilityStatus } from '../compatibility';
import type { OcfZipLimits } from '../archive';
...
export async function runEpubCorpusCase(test: EpubCorpusCase): Promise<EpubCorpusCaseResult> {
  const loaded = await loadEpub(test.bytes, { archiveLimits: test.archiveLimits });
  ...
  if (test.expectPublication != null && opened !== test.expectPublication) {
    failures.push(`expected publicationOpened=${test.expectPublication}, got ${opened}`);
  }
  ...
  if (test.expectedCompatibilityStatus && compatibility.status !== test.expectedCompatibilityStatus) {
    failures.push(`expected compatibility=${test.expectedCompatibilityStatus}, got ${compatibility.status}`);
  }
...
export async function runEpubCorpus(cases: readonly EpubCorpusCase[]): Promise<readonly EpubCorpusCaseResult[]> {
