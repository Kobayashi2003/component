import { describeReaderPosition } from '../accessibility';
import { MemoryReaderMarkStore, ReaderMarkController, type ReaderMark, type ReaderMarkStore } from '../annotations';
import { ReaderThemeRegistry } from '../appearance';
import { OcfZipArchive } from '../archive';
import { createCompatibilityReport } from '../compatibility';
import { BrowserDomXmlPlatform, preflightPublicationContent } from '../content';
import { ReaderDecorationController } from '../decorations';
import { BrowserReaderInputRouter, ReaderInputController } from '../input';
import { locatorAtResourceStart } from '../locator';
import { BrowserPublicationMediaRouter } from '../media';
import { loadPublicationFootnote, locatorFromCfi, locatorFromHref, PublicationLinkRouter, ReaderNavigationHistory, ReaderNavigator, type NavigationPlanProvider, type NavigationTarget, type ReaderNavigationResult } from '../navigation';
import {
...
import { BrowserObjectUrlFactory, PublicationResourceSession, ResourceResolver } from '../resources';
import { BrowserPublicationSearchProvider, PublicationSearch, ReaderSearchController, type SearchOptions } from '../search';
import { BrowserReaderSelectionRouter, captureSelectionFromDocuments, getDocumentSelection, type ReaderSelection } from '../selection';
import type {
...
export class BrowserEpubReader {
  ...
  private constructor(
    readonly publication: import('../publication').Publication,
  ...
  async addHighlightFromSelection(
    highlight: import('../annotations').AnnotationHighlightStyle = 'solid',
    color: import('../annotations').AnnotationColor = 'yellow',
  ): Promise<import('../annotations').Highlight | null> {
  ...
  registerTheme(theme: import('../appearance').ReaderThemeDefinition): void {
    this.assertAlive();
  ...
  private inputState(): import('../input').ReaderInputState {
  ...
  private async searchWithHistory(operation: () => Promise<import('../search').SearchHit | null>): Promise<import('../search').SearchHit | null> {
    const origin = await this.currentHistoryLocator();
...
function emptyRendererState(): import('../renderer').RendererHostState {
  return { status: 'idle', generation: 0, plan: null, rendererKind: null, layout: null, stability: null, error: null };
}

function emptySearchState(): import('../search').ReaderSearchState {
