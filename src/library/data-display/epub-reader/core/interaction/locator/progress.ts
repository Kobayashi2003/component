...
export function fixedLayoutPublicationProgress(spineIndex: number, spineCount: number): number {
...
export function locationForPublicationProgress(
  ...
  if (!Number.isInteger(spineCount) || spineCount <= 0) return { spineIndex: 0, progression: 0 };
