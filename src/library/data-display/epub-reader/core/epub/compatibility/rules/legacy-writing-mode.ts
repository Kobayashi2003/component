...
export const LEGACY_WRITING_MODE_COMPATIBILITY_ID = 'epub.content.legacy-writing-mode';
...
export const legacyWritingModeCompatibilityRule: ContentDocumentCompatibilityRule = {
  ...
  apply(context, state) {
    ...
    if (candidate?.writingModeSource !== 'legacy' || !candidate.writingMode) return { value: state };
    return {
      value: {
        ...state,
        hints: {
