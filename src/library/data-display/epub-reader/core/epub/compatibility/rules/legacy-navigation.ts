...
export const LEGACY_NAVIGATION_COMPATIBILITY_ID = 'epub.publication.legacy-navigation';
  ...
  apply(context, navigation) {
    ...
    if (legacy) {
      const value: NavigationModel = {
        ...legacy,
        landmarks: legacy.landmarks.length > 0 ? legacy.landmarks : context.legacyLandmarks,
      };
      const diagnostics = context.publication.version.startsWith('3')
        ? [{
          ...
          message: 'EPUB 3 Navigation Document did not provide a usable table of contents; using legacy NCX navigation.',
          path: context.publication.packagePath,
          repair: {
            strategy: 'use-ncx-navigation-fallback',
            description: 'Use declared NCX navigation when EPUB 3 navigation is missing or unusable.',
            confidence: 0.7,
