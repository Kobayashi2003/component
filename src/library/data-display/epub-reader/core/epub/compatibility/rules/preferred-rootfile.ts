...
export const PREFERRED_ROOTFILE_COMPATIBILITY_ID = 'epub.publication.preferred-rootfile';
...
export const preferredRootfileCompatibilityRule: RootfileSelectionCompatibilityRule = {
  ...
  apply(context, selected) {
    const preferred = context.rootfiles.find(rootfile => rootfile.mediaType === PACKAGE_MEDIA_TYPE) ?? selected;
    ...
    return {
      ...
      diagnostics: [{
        ...
        repair: {
          ...
          description: 'Prefer the first application/oebps-package+xml rootfile over a non-package candidate.',
          confidence: 0.8,
        },
      }],
    };
