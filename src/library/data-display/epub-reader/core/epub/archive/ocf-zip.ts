...
import { normalizePublicationPath, validateArchiveEntryPath } from '../publication/path';
...
export class OcfZipArchive implements PublicationArchive {
  ...
  static async open(
    ...
    try {
      ...
      if (mode === 'strict' && diagnostics.some(diagnostic => diagnostic.severity === 'error' || diagnostic.severity === 'fatal')) {
        return { archive: null, diagnostics };
      }
      ...
      diagnostics.push({
        ...
        severity: 'fatal',
        phase: 'archive',
        message: cause instanceof Error ? cause.message : 'Failed to open EPUB ZIP container.',
        cause,
      });
      return { archive: null, diagnostics };
    }
  ...
  async read(path: PublicationPath): Promise<Uint8Array> {
    ...
    if ((entry.flags & 0x1) !== 0) throw new Error(`Encrypted ZIP entry is not supported: ${normalized}`);
    ...
    if (view.getUint32(offset, true) !== SIG_LOCAL) throw new Error(`Invalid local ZIP header: ${normalized}`);
    ...
    else throw new Error(`Unsupported ZIP compression method ${entry.compression} for ${normalized}`);

    if (output.byteLength !== entry.uncompressedSize) {
      throw new Error(`Uncompressed size mismatch for ${normalized}: expected ${entry.uncompressedSize}, got ${output.byteLength}`);
    }
...
function readCentralDirectory(
  ...
  if (eocdOffset < 0) {
    diagnostics.push(diag('OCF_ZIP_EOCD_MISSING', 'fatal', 'ZIP end-of-central-directory record was not found.'));
    return null;
  }
  ...
  if (disk !== 0 || centralDisk !== 0) {
    diagnostics.push(diag('OCF_ZIP_MULTIDISK_UNSUPPORTED', 'fatal', 'Multi-disk ZIP containers are not supported.'));
    return null;
  }
  ...
  if (needsZip64) {
    ...
    if (locatorOffset < 0 || view.getUint32(locatorOffset, true) !== SIG_ZIP64_LOCATOR) {
      diagnostics.push(diag('OCF_ZIP64_LOCATOR_MISSING', 'fatal', 'ZIP64 locator is missing.'));
      return null;
    }
    const zip64Offset = bigintToSafeNumber(view.getBigUint64(locatorOffset + 8, true), 'ZIP64 EOCD offset');
    ensureRange(bytes, zip64Offset, 56);
    if (view.getUint32(zip64Offset, true) !== SIG_ZIP64_EOCD) {
      diagnostics.push(diag('OCF_ZIP64_EOCD_INVALID', 'fatal', 'ZIP64 end-of-central-directory record is invalid.'));
      return null;
    }
    ...
    if (zip64Disk !== 0 || zip64CentralDisk !== 0) {
      diagnostics.push(diag('OCF_ZIP64_MULTIDISK_UNSUPPORTED', 'fatal', 'Multi-disk ZIP64 containers are not supported.'));
      return null;
    }
    entryCount = bigintToSafeNumber(view.getBigUint64(zip64Offset + 32, true), 'ZIP64 entry count');
    centralSize = bigintToSafeNumber(view.getBigUint64(zip64Offset + 40, true), 'ZIP64 central size');
    centralOffset = bigintToSafeNumber(view.getBigUint64(zip64Offset + 48, true), 'ZIP64 central offset');
  }
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (view.getUint32(cursor, true) !== SIG_CENTRAL) {
      diagnostics.push(diag('OCF_ZIP_CENTRAL_INVALID', 'fatal', `Invalid central directory entry at index ${index}.`));
      return null;
    }
    ...
    if ((flags & 0x1) !== 0) {
      diagnostics.push(diag('OCF_ZIP_ENTRY_ENCRYPTED', 'error', `Entry ${name} is ZIP-encrypted.`, normalizePublicationPath(name)));
    }
    ...
    if (!name.endsWith('/')) {
      ...
      try {
        ...
        diagnostics.push({
          ...diag('OCF_ZIP_ENTRY_PATH_INVALID', 'error', `ZIP entry has an invalid OCF path: ${name}.`),
          cause,
        });
      ...
      if (entries.some(entry => entry.path === path)) {
        diagnostics.push(diag('OCF_ZIP_ENTRY_DUPLICATE', 'error', `ZIP contains a duplicate entry path: ${path}.`, path));
        cursor += 46 + nameLength + extraLength + commentLength;
        continue;
      }
      entries.push({
...
async function validateOcfMimetype(
  ...
  const firstByOffset = [...entries].sort((a, b) => a.localHeaderOffset - b.localHeaderOffset)[0];
  if (!firstByOffset || firstByOffset.path !== 'mimetype') {
    diagnostics.push(mode === 'compatible'
      ? {
          code: 'OCF_MIMETYPE_NOT_FIRST',
          severity: 'warning',
          phase: 'archive',
          message: 'The `mimetype` file is not the first ZIP entry; compatibility mode accepts the otherwise valid stored mimetype.',
          repair: {
            strategy: 'accept-misordered-ocf-mimetype',
            description: 'Accept a valid uncompressed EPUB mimetype entry even when its ZIP ordering violates OCF.',
            confidence: 0.99,
          },
        }
      : diag('OCF_MIMETYPE_NOT_FIRST', 'error', 'The `mimetype` file is not the first ZIP entry.'));
  }

  const mimetype = entries.find(entry => entry.path === 'mimetype');
  if (!mimetype) {
    diagnostics.push(diag('OCF_MIMETYPE_MISSING', 'error', 'The required `mimetype` file is missing.'));
    return;
  }
  if (mimetype.compression !== 0) {
    diagnostics.push(diag('OCF_MIMETYPE_COMPRESSED', 'error', 'The `mimetype` file must be stored without compression.'));
  }

  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  ensureRange(bytes, mimetype.localHeaderOffset, 30);
  const extraLength = view.getUint16(mimetype.localHeaderOffset + 28, true);
  if (extraLength !== 0) diagnostics.push(diag('OCF_MIMETYPE_HAS_EXTRA_FIELD', 'error', 'The `mimetype` local header must not contain an extra field.'));

  try {
    const value = await archive.readText('mimetype', 'ascii');
    if (value !== 'application/epub+zip') {
      diagnostics.push(diag('OCF_MIMETYPE_INVALID', 'error', `Unexpected EPUB mimetype value: ${JSON.stringify(value)}.`));
    }
  } catch (cause) {
    diagnostics.push({
      ...diag('OCF_MIMETYPE_READ_FAILED', 'error', 'Failed to read the EPUB `mimetype` file.'),
      cause,
    });
  }
}
...
function parseZip64Extra(
  extra: Uint8Array,
  requested: { uncompressedSize: boolean; compressedSize: boolean; localHeaderOffset: boolean },
  ...
  while (cursor + 4 <= extra.length) {
    ...
    if (requested.uncompressedSize) {
      ...
      out.uncompressedSize = bigintToSafeNumber(view.getBigUint64(cursor, true), 'ZIP64 uncompressed size');
      cursor += 8;
    }
    if (requested.compressedSize) {
      if (cursor + 8 > end) return null;
      out.compressedSize = bigintToSafeNumber(view.getBigUint64(cursor, true), 'ZIP64 compressed size');
      cursor += 8;
    }
    if (requested.localHeaderOffset) {
      if (cursor + 8 > end) return null;
      out.localHeaderOffset = bigintToSafeNumber(view.getBigUint64(cursor, true), 'ZIP64 local header offset');
    }
    return out;
  }
  return null;
...
function decodeZipName(
  ...
  try {
    ...
    if ((flags & 0x800) === 0 && bytes.some(byte => byte >= 0x80)) {
      diagnostics.push(diag('OCF_ZIP_UTF8_FLAG_MISSING', 'warning', `Non-ASCII ZIP name ${name} does not set the UTF-8 language flag.`));
    }
    return name;
  } catch {
    diagnostics.push(diag('OCF_ZIP_FILENAME_INVALID_UTF8', 'error', 'ZIP entry name is not valid UTF-8.'));
...
function ensureRange(bytes: Uint8Array, offset: number, length: number): void {
  if (!Number.isInteger(offset) || !Number.isInteger(length) || offset < 0 || length < 0 || offset + length > bytes.byteLength) {
    throw new Error(`ZIP structure points outside container bounds (${offset}+${length}/${bytes.byteLength}).`);
  }
}
