...
export function parseReadingSessionRecord(value: unknown): ReadingSessionRecord | null {
  if (!isRecord(value) || !hasExactKeys(value, ['locator', 'marks', 'updatedAt'], ['preferences'])) return null;
  const record = value as unknown as Partial<ReadingSessionRecord>;
  if (!isLocator(record.locator)) return null;
  if (record.preferences != null && !isReaderPreferences(record.preferences)) return null;
...
function isReaderMark(value: unknown): value is ReaderMark {
  ...
  if (!isNonEmptyString(mark.id) || !isTimestamp(mark.createdAt) || !isTimestamp(mark.updatedAt)) return false;
  if (mark.label !== undefined && typeof mark.label !== 'string') return false;
  if (mark.tags !== undefined && (!Array.isArray(mark.tags) || !mark.tags.every(tag => typeof tag === 'string'))) return false;
  const optional = ['label', 'tags'] as const;
  if (mark.kind === 'bookmark') {
    return hasExactKeys(value, ['id', 'kind', 'locator', 'createdAt', 'updatedAt'], optional)
      && isLocator(mark.locator);
  }
  if (mark.kind !== 'highlight' && mark.kind !== 'annotation') return false;
  const range = mark.range;
  if (!isRecord(range) || !hasExactKeys(range, ['start', 'end']) || !isLocator(range.start) || !isLocator(range.end)) return false;
  if (!isAnnotationColor(mark.color) || !isAnnotationHighlightStyle(mark.highlight)) return false;
  return mark.kind === 'highlight'
    ? hasExactKeys(value, ['id', 'kind', 'range', 'color', 'highlight', 'createdAt', 'updatedAt'], optional)
    : hasExactKeys(value, ['id', 'kind', 'range', 'body', 'color', 'highlight', 'createdAt', 'updatedAt'], optional)
      && typeof value.body === 'string';
}

function isLocator(value: unknown): value is Locator {
  if (!isRecord(value) || !hasExactKeys(value, ['href', 'spineIndex', 'locations'], ['text'])) return false;
  const locator = value as Partial<Locator>;
  return isNonEmptyString(locator.href)
...
function isLocatorLocations(value: unknown): boolean {
  if (!isRecord(value) || !hasExactKeys(value, [], ['cfi', 'fragment', 'dom', 'progression', 'position'])) return false;
  if (value.cfi !== undefined && !isNonEmptyString(value.cfi)) return false;
  if (value.fragment !== undefined && typeof value.fragment !== 'string') return false;
  if (value.progression !== undefined && (!isFiniteNumber(value.progression) || value.progression < 0 || value.progression > 1)) return false;
  if (value.position !== undefined && (!Number.isInteger(value.position) || (value.position as number) < 0)) return false;
  if (value.dom === undefined) return true;
  return isRecord(value.dom)
...
function isLocatorText(value: unknown): boolean {
  return isRecord(value)
    ...
    && ['before', 'highlight', 'after'].every(key => value[key] === undefined || typeof value[key] === 'string');
}

function isReaderPreferences(value: unknown): value is ReaderPreferences {
  if (!isRecord(value) || !hasExactKeys(value, [
    'flow', 'spread', 'pageProgression', 'fontSizePercent', 'fontFamily', 'lineHeight',
    'pageMarginPercent', 'fixedLayoutFit', 'fixedLayoutGutter', 'touchNavigation',
    'pageTurnZonePercent', 'compatibility', 'theme',
  ])) return false;
  return isOneOf(value.flow, ['auto', 'paginated', 'scrolled'])
...
function isCompatibilityPreferences(value: unknown): boolean {
  const keys = [
    'recoverContainerStructure', 'selectPreferredRootfile', 'recoverMalformedXhtml',
    'useLegacyNavigationFallback', 'normalizeLegacyCss', 'fitSingleImagePages',
...
function isAnnotationColor(value: unknown): boolean {
  return isOneOf(value, ['pink', 'orange', 'yellow', 'green', 'blue', 'purple']);
}
