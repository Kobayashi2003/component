...
const PLACEMENTS = new Set<ReaderToolPlacement>(['navigation', 'primary', 'secondary']);
...
export function createReaderToolRegistry(modules: readonly ReaderToolModule[]): ReaderToolRegistry {
  ...
  const validated = modules.map((candidate, index) => {
    ...
    if (byId.has(candidate.id)) throw new TypeError(`Duplicate Reader tool id: ${candidate.id}.`);
    for (const key of ['label', 'shortLabel', 'description'] as const) {
      const value = candidate[key];
      if (typeof value !== 'string' || !value.trim() || value.length > 160) {
        throw new TypeError(`Reader tool ${candidate.id} ${key} must be a non-empty string of at most 160 characters.`);
      }
    }
    if (!PLACEMENTS.has(candidate.placement)) {
      throw new TypeError(`Reader tool ${candidate.id} has an unsupported placement: ${String(candidate.placement)}.`);
    }
    if (candidate.ariaKeyShortcuts != null
      && (typeof candidate.ariaKeyShortcuts !== 'string' || !candidate.ariaKeyShortcuts.trim())) {
      throw new TypeError(`Reader tool ${candidate.id} ariaKeyShortcuts must be a non-empty string.`);
    }
    if (candidate.command != null) {
      if (!COMMANDS.has(candidate.command)) {
        throw new TypeError(`Reader tool ${candidate.id} has an unsupported command: ${String(candidate.command)}.`);
      }
      if (byCommand.has(candidate.command)) {
        throw new TypeError(`Reader command ${candidate.command} is already owned by another tool.`);
      }
    }
    if (candidate.isAvailable != null && typeof candidate.isAvailable !== 'function') {
      throw new TypeError(`Reader tool ${candidate.id} isAvailable must be a function.`);
    }
    if (typeof candidate.renderIcon !== 'function' || typeof candidate.render !== 'function') {
      throw new TypeError(`Reader tool ${candidate.id} must provide renderIcon and render functions.`);
    }
  ...
  return Object.freeze({
    ...
    available: (context: Pick<ReaderToolContext, 'reader'>) => Object.freeze(
