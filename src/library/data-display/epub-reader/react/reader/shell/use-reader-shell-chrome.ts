...
import { useEpubReaderFullscreen, type EpubReaderFullscreenController } from '../../chrome/use-epub-reader-fullscreen';
...
interface ReaderShellChromeOptions {
  ...
  readonly actionsRef: { current: Pick<ReaderChromeControls, 'show' | 'toggle'> | null };
}
...
export function useReaderShellChrome(options: ReaderShellChromeOptions): ReaderShellChromeController {
  const { shellRef, hasPublicationSnapshot, surfaceOpen, onFullscreenError, actionsRef } = options;
  const [pointerOverChrome, setPointerOverChrome] = useState(false);
  const [focusInChrome, setFocusInChrome] = useState(false);
  const chrome = useReaderChrome(shouldLockReaderChrome({
    hasPublicationSnapshot,
    surfaceOpen,
    pointerOverChrome,
    focusInChrome,
  }));
  const fullscreen = useEpubReaderFullscreen(shellRef, { onError: onFullscreenError });
  ...
  useEffect(() => {
    ...
    const updateFocusLock = () => {
      ...
      setFocusInChrome(Boolean(active && (toolbar?.contains(active) || controls?.contains(active))));
    };
    ...
    toolbar?.addEventListener('pointerenter', holdPointerLock, { passive: true });
    toolbar?.addEventListener('pointerleave', releasePointerLock, { passive: true });
    controls?.addEventListener('pointerenter', holdPointerLock, { passive: true });
    controls?.addEventListener('pointerleave', releasePointerLock, { passive: true });
