...
export function PageLayoutSettings({
  ...
  return (
    <div className="epub-settings-panel__section">
      ...
      {showFlow ? <label className="epub-settings-panel__select-row">
        <span><strong>Flow</strong><small>Choose paging or continuous scrolling.</small></span>
        <select value={preferences.flow} onChange={(event: ChangeEvent<HTMLSelectElement>) => void reader.setPreferences({ flow: event.currentTarget.value as ReaderPreferences['flow'] })}>
      ...
      <label className="epub-settings-panel__select-row">
        ...
        <select value={preferences.spread} onChange={(event: ChangeEvent<HTMLSelectElement>) => void reader.setPreferences({ spread: event.currentTarget.value as ReaderPreferences['spread'] })}>
          <option value="auto">Auto</option>
      ...
      <label className="epub-settings-panel__select-row">
        ...
        <select value={preferences.pageProgression} onChange={(event: ChangeEvent<HTMLSelectElement>) => void reader.setPreferences({ pageProgression: event.currentTarget.value as ReaderPreferences['pageProgression'] })}>
          <option value="auto">Auto</option>
