...
const COMPATIBILITY_GROUPS = [
  {
    ...
    items: [
      { key: 'recoverContainerStructure', label: 'Recover container structure', description: 'Recoverable OCF/ZIP deviations → readable EPUB container.' },
      { key: 'selectPreferredRootfile', label: 'Select preferred package', description: 'Several rootfiles → the first standard OPF package.' },
      { key: 'recoverMalformedXhtml', label: 'Recover malformed XHTML', description: 'Non-well-formed XHTML → browser HTML parsing.' },
    ],
  },
  {
    title: 'Navigation',
    items: [
      { key: 'useLegacyNavigationFallback', label: 'Use legacy navigation', description: 'Missing EPUB 3 navigation → EPUB 2 NCX and Guide.' },
    ],
  },
  {
    title: 'Layout',
    items: [
      { key: 'normalizeLegacyCss', label: 'Normalize legacy CSS', description: 'Legacy -epub/-webkit declarations → standard CSS.' },
      { key: 'fitSingleImagePages', label: 'Fit single-image pages', description: 'Image-only reflowable documents → one contained page.' },
    ],
  },
  {
    title: 'Resources',
    items: [
      { key: 'deobfuscateIdpfFonts', label: 'Decode IDPF fonts', description: 'IDPF-obfuscated fonts → browser-loadable fonts.' },
    ],
  },
