...
`core/runtime/configuration/` is the host-facing composition boundary for the extension points that are implemented end to end. `configureReaderExtensions` validates three explicit contribution buckets—EPUB compatibility modules, input bindings, and themes—and combines them with built-ins before a publication opens. It never turns the internal lifecycle machinery into a generic plugin API.
...
`core/epub/compatibility/` owns Compatibility Profiles and four phase-specific module families for publication selection, content-document processing, bounded resource transformation, and closed rendition directives. `compatibility/rules/` contains the registered built-ins for preferred rootfile selection, NCX/Guide fallback, malformed XHTML recovery, legacy writing-mode interpretation, legacy CSS aliases, IDPF font deobfuscation, and single-image fitting. Profiles resolve explicit enable/disable choices and same-stage dependencies into a deterministic signature suitable for cache variants.

A Compatibility Profile is created once when a Publication is opened and remains immutable for that Reading Session. Compatibility preference changes are staged in the Reader Snapshot for the next reopen; they do not partially change an already parsed publication. Package loading, preflight, resource sessions, content parsing, search and rendition planning all receive that same Profile.
...
`core/interaction/` owns locators, publication-progress conversion, navigation, input routing, and selection. The browser input router permanently owns DOM listeners, focus recovery, scroll containment, selection protection, thresholds, and gesture recognition. Its configurable `ReaderInputMap` receives only normalized signals and may return only closed `ReaderCommand` values. Input bindings are priority ordered, invalid or failing contributions are isolated, and their shortcut descriptions drive the React keyboard-help surface.

React controls display progress but do not define how a publication percentage maps back to a spine item and section progression. Presentation and interaction are collaborating peers: renderers resolve and capture locators, while navigation drives renderer contracts. Neither is modeled as a false one-way abstraction over the other.
...
`core/extension/` contains internal controlled-extension primitives: deterministic dependency ordering, lifecycle ownership, typed Capability registration, and failure-isolated Reader Event fan-out. It is not a generic public plugin entry point. EPUB compatibility may share its inert ordering model, but uses separate phase-specific contracts rather than the Feature lifecycle runner.

Search asks the publication-scoped `PublicationContentDocumentPipeline` to parse a reading-order item only long enough to create a pure-data text and locator index; cached entries contain CFI paths, DOM paths, text offsets, and diagnostics, never `Document` or `Text` nodes. The same pipeline supplies parsed documents to render materialization, preventing malformed-XHTML handling from drifting between search and rendering. A count-and-byte-budgeted LRU favours the current spine item and its neighbours. Query cancellation does not discard shared index work needed by a newer query, while explicit cache clearing and reader disposal abort pending builds and release all completed indexes.

Rendering has a separate publication-scoped content-document cache. It retains frozen serialized markup, presentation hints, and diagnostics so a recent chapter revisit does not repeat archive reading, parsing, resource rewriting, or serialization. Its keys include the content/resource Compatibility Profile signature; search-index keys include the content signature. It never retains a live `Document`, iframe, or renderer instance. Count and approximate-byte limits bound markup memory, while generated and rewritten object URLs remain under `PublicationResourceSession` ownership and are revoked once, after renderers and the content cache are disposed.
...
Within the engine, EPUB processing and reading Features cannot depend on the reader runtime, and the generic extension mechanism cannot depend on concrete Features. The runtime remains the composition root. The accepted controlled-extension target and the intentionally sealed Kernel boundaries are recorded in [ADR 0001](./adr/0001-controlled-reader-extension-boundaries.md). Only mechanisms already wired through their complete lifecycle are exposed by `runtime/configuration`; publication-scoped Feature registration remains internal.

### Styling

`core/presentation/appearance/` owns the validated Theme Catalog. A theme may provide publication colors, a preview, and an optional closed host-UI token palette; it cannot inject arbitrary rules or manipulate components. The active catalog is exposed through the Reader Snapshot, so React settings no longer maintain a second theme list. Registering a theme used by the active preference performs a normal renderer relayout.
