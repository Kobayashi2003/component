...
function fakePipeline(
  ...
  } as unknown as PublicationContentDocumentPipeline;
}
...
async function main() {
  ...
  const pendingCache = new PublicationContentDocumentCache(
    fakePipeline(),
    {},
    pendingMaterializer,
  );
  ...
  const variantMaterializer: PublicationContentDocumentMaterializer = async (
    item,
  ) => {
  ...
  const boundedMaterializer: PublicationContentDocumentMaterializer = async (
    item,
  ) => {
    boundedLoads[item.index] = (boundedLoads[item.index] ?? 0) + 1;
    return result(item);
  };
  const boundedCache = new PublicationContentDocumentCache(
    fakePipeline(),
    {
  ...
  const clearingCache = new PublicationContentDocumentCache(
    fakePipeline(),
    {},
    () =>
      new Promise((resolve) => {
