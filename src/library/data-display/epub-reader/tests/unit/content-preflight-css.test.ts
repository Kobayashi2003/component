import {
  ...
  type PublicationArchive,
} from "../../core/epub/archive";
import {
  preflightPublicationContent,
  PublicationContentPreflightSession,
} from "../../core/epub/content/preflight";
import type {
  Publication,
  PublicationPath,
  WritingMode,
...
const CASES: readonly PreflightCase[] = [
  {
    ...
    css: {
      "base.css":
        "body{writing-mode:vertical-rl}body.imgpage{writing-mode:horizontal-tb}",
  ...
  },
  {
    ...
    css: {
      "a.css": "body{writing-mode:horizontal-tb}",
      "b.css": "body{writing-mode:vertical-rl}",
    },
  ...
  },
  {
    ...
    css: {
      "v.css": "body{writing-mode:vertical-rl}",
      "h.css": "body{writing-mode:horizontal-tb}",
    },
    expected: "vertical-rl",
  },
  {
    ...
    css: {
      "a.css":
        "body{writing-mode:vertical-rl}@media print{body{writing-mode:horizontal-tb}}",
    },
    expected: "vertical-rl",
  },
  {
  ...
  },
  {
    ...
    css: {
      "a.css":
        "@font-face{font-family:x;src:url(x.otf)}@keyframes k{from{writing-mode:horizontal-tb}}body{writing-mode:vertical-rl}",
    },
    expected: "vertical-rl",
  },
  {
  ...
  },
  {
    ...
    css: {
      "a.css":
        "body{writing-mode:vertical-rl !important}body.imgpage{writing-mode:horizontal-tb}",
    },
    expected: "vertical-rl",
  },
  {
...
function stagedPublication(count: number): Publication {
  const documents = Array.from(
    { length: count },
    (_, index) => `EPUB/doc-${index}.xhtml` as PublicationPath,
  );
  return {
...
async function main() {
  ...
  for (const testCase of CASES) {
    const contents: Record<string, string> = {
      "EPUB/doc.xhtml": `<?xml version="1.0"?>${testCase.html.replace("<html", '<html xmlns="http://www.w3.org/1999/xhtml"')}`,
    };
    for (const [name, css] of Object.entries(testCase.css))
      contents[`EPUB/${name}`] = css;
    const archive = new MemoryPublicationArchive(contents);
    const publication = publicationFor(
      Object.keys(contents) as PublicationPath[],
    );
    ...
    if (resolved !== testCase.expected) {
      failures.push(
        `${testCase.name}: expected ${testCase.expected ?? "no hint"}, got ${resolved ?? "no hint"}`,
      );
    }
  }
  assert(
    failures.length === 0,
    `preflight CSS subset regressions:\n  ${failures.join("\n  ")}`,
  );

  const staged = stagedPublication(5);
  const contents = Object.fromEntries(
    staged.spine.map((item) => [
      item.path!,
  ...
  assert(
    critical.hints.size === 3,
    "a render-critical preflight must return only the requested spine window",
  );
  assert(
    staged.spine.filter((item) => reads.has(item.path!)).length === 3,
    "opening work must not inspect distant spine items",
  );
  assert(
    reads.get("EPUB/shared.css") === 1,
    "staged items must share stylesheet reads",
  );

  await Promise.all([session.inspect([4]), session.inspect([4])]);
  assert(
    reads.get("EPUB/doc-4.xhtml") === 1,
    "concurrent background and navigation requests must share one item inspection",
  );
  const complete = await session.inspect();
  assert(
    complete.hints.size === 5,
    "the background pass must complete the same full publication profile",
  );
  assert(
    staged.spine.every((item) => reads.get(item.path!) === 1),
    "each content document must be inspected at most once across all stages",
  );
  session.dispose();
  const disposed = await session.inspect([0]).then(
    () => null,
    (error) => error,
  );
  assert(
    disposed instanceof Error && disposed.message.includes("disposed"),
    "disposing a preflight session must reject new work",
  );
  ...
  const abortingSession = new PublicationContentPreflightSession(
    delayedArchive,
    staged,
    abortController.signal,
  );
  const aborted = abortingSession.inspect([0]).then(
    () => null,
    (error) => error,
  );
  ...
  assert(
    ...
    "replacing a reader must cancel unfinished staged preflight",
  );
  abortingSession.dispose();

  const openingController = new AbortController();
  const detachedSession = new PublicationContentPreflightSession(
    memory,
    staged,
  ...
  assert(
    afterOpen.hints.has(0),
    "an opening signal must stop owning background preflight after the reader becomes ready",
  );
  detachedSession.dispose();

  console.log(
    `Content preflight CSS subset unit test: PASS (${CASES.length} cases)`,
  );
}
