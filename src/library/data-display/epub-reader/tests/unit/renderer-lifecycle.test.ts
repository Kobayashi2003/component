...
async function main(): Promise<void> {
  ...
  // 2. Geometry must settle for consecutive frames after font/image readiness.
  {
    ...
    const target: LayoutStabilityTarget = {
      async waitForFonts() {},
      async decodeImages() {
        return { decoded: 2, failed: 1, total: 3 };
      },
      measure() {
        return measurements[Math.min(index, measurements.length - 1)]!;
      },
    ...
    const report = await waitForLayoutStability(
      target,
  ...
  class FakeRenderer implements RendererInstance {
    ...
    snapshot() {
      return {
        pageCount: Math.round(this.committedFontSize / 10),
        currentPage: 1,
      };
    }
