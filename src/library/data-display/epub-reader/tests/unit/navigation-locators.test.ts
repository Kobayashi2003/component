...
const publication: Publication = {
  ...
  metadata: { title: 'Navigation fixture', creators: [], contributors: [], entries: [] },
...
const plans = publication.spine.map(item => planRendition({
  publication,
  spineItem: item,
...
class FakeHost implements NavigationRendererHost {
  state: RendererHostState = {
    status: 'idle', generation: 0, plan: null, rendererKind: null,
    layout: null, stability: null, error: null,
  };
  ...
  async navigateWithin(): Promise<RendererNavigationResult> {
    ...
    this.maxConcurrentNavigations = Math.max(this.maxConcurrentNavigations, this.activeNavigations);
    if (this.navigationDelayMs > 0) {
      await new Promise<void>(resolve => setTimeout(resolve, this.navigationDelayMs));
    }
    this.activeNavigations -= 1;
    return this.within;
  }
...
async function main() {
  ...
  assert(escaped === 'a^[b^]^^c^,d^;e^(f^)', 'CFI assertions must circumflex-escape delimiters');

  const parsed = parseEpubCfi('epubcfi(/6/4[spine-1]!/4[body]/2/1:3)');
  assert(parsed.packagePath.steps[1]?.index === 4, 'CFI package itemref step should parse');
  assert(parsed.contentPath.characterOffset === 3, 'CFI UTF-16 character offset should parse');
  assert(serializeCfiPath(parsed.contentPath) === '/4[body]/2/1:3', 'CFI point path should round-trip');
  assert(resolveCfiSpineItem(publication, parsed.packagePath).index === 1, 'CFI package path should resolve spine item');

  const assertedPoint = parseEpubCfi('epubcfi(/6/4[spine-1]!/4[body]/2/1:3[abc,def;s=b])');
  assert(assertedPoint.contentPath.textAssertion?.before === 'abc', 'CFI terminal before assertion should parse');
  assert(assertedPoint.contentPath.textAssertion?.after === 'def', 'CFI terminal after assertion should parse');
  assert(assertedPoint.contentPath.sideBias === 'before', 'CFI side bias should parse');
  assert(serializeCfiPath(assertedPoint.contentPath) === '/4[body]/2/1:3[abc,def;s=b]', 'CFI terminal assertion should round-trip');

  const rangeAsPoint = parseEpubCfi('epubcfi(/6/4[spine-1]!/4/2,/1:1,/1:5)');
  assert(rangeAsPoint.contentPath.characterOffset === 1, 'a CFI range used as a point must resolve to its start');

  const correctedByAssertion = parseEpubCfi('epubcfi(/8/99[spine-2]!/4/2/1:0)');
  assert(resolveCfiSpineItem(publication, correctedByAssertion.packagePath).index === 2, 'itemref ID assertion should recover a structurally stale package CFI');

  const hrefLocator = locatorFromHref(publication, 'EPUB/c2.xhtml#section-4');
  assert(hrefLocator.spineIndex === 2 && hrefLocator.locations.fragment === 'section-4', 'href navigation should preserve fragment');

  const cfiLocator = locatorFromCfi(publication, 'epubcfi(/6/8[spine-3]!/4/2/1:0)');
  assert(cfiLocator.spineIndex === 3, 'CFI navigation target should resolve without opening content document');

  const history = new ReaderNavigationHistory(2);
  const origin: Locator = { href: 'EPUB/c0.xhtml', spineIndex: 0, locations: { progression: 0.25 } };
  const destination: Locator = { href: 'EPUB/c2.xhtml', spineIndex: 2, locations: { fragment: 'section-4' } };
  history.record(origin, destination);
  assert(history.snapshot.canGoBack && history.peekBack()?.spineIndex === 0, 'branch navigation should retain its origin');
  history.commitBack(destination);
  assert(history.snapshot.canGoForward && history.peekForward()?.spineIndex === 2, 'going back should make the previous destination available to go forward');
  history.commitForward(origin);
  history.record(destination, origin);
  assert(!history.snapshot.canGoForward, 'a new branch navigation should clear forward history');
  ...
  assert(forward.status === 'moved' && forward.spineChanged, 'boundary navigation should move into another spine item');
  assert(Number(host.state.plan?.spineIndex) === 2, 'sequential navigation should skip linear="no" item');
  assert(Number(host.locator?.locations.progression) === 0, 'forward cross-spine navigation should enter at resource start');

  host.within = { status: 'boundary', edge: 'start' };
  const back = await navigator.previous();
  assert(back.status === 'moved' && Number(host.state.plan?.spineIndex) === 0, 'backward navigation should skip non-linear item too');
  assert(Number(host.locator?.locations.progression) === 1, 'backward cross-spine navigation should enter at resource end');
  ...
  host.state = { ...host.state, plan: plans[0]!, layout: { pageCount: 4, currentPage: 1 } };
