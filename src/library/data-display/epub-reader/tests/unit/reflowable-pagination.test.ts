...
// arbitrary generated-location sampling interval.
{
  ...
  assert(
    geometry.pageCount === 5,
    "five physical columns must produce five virtual pages",
  );
  assert(
    geometry.currentPage === 3,
    "offset of two page advances must select page three",
  );
  assert(
    geometry.progression === 0.5,
    "middle of five pages must report 50% progression",
  );
  assert(
    geometry.snappedOffset === 1664,
    "page geometry must snap to an exact page advance",
  );
  assert(
    pageOffsetForProgression(1, 5, 832) === 3328,
    "100% progression must map to the last page",
  );
  assert(
    scrollProgression(500, 1000) === 0.5,
    "scroll progression must be continuous and normalized",
  );
}
...
// 1a. An exact two-column extent must not acquire a third phantom page.
{
  ...
  assert(
    ...
    "exact column extents must not report a phantom trailing page",
  );
}
...
// writing uses its physical width as CSS multicol inline size.
{
  ...
  assert(
    plan.viewport.width === 800 && plan.viewport.height === 600,
    "planner must retain viewport metrics",
  );
  const css = buildReflowableLayoutCss(
    plan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
    "horizontal-tb",
  );
  assert(
    css.includes("column-width: 800px"),
    "horizontal writing column inline size must use page width",
  );
  assert(
    css.includes("column-fill: auto"),
    "paginated reflow must fill fixed-height columns sequentially",
  );
}
...
// window; the stacking was right, the horizontal transport under it was wrong.
{
  ...
  assert(
    ...
    "double reflowable text should remain one spine document",
  );
  const css = buildReflowableLayoutCss(
    plan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
  ...
  assert(
    css.includes("column-fill: auto"),
    "vertical pagination must fragment through CSS multicol",
  );
  assert(
    !/column-width: auto/.test(css),
    "vertical pagination must give columns a definite inline size",
  );
  assert(
    css.includes("overflow-y: auto"),
    "vertical column boxes stack on Y, so paging scrolls Y",
  );
  assert(
    css.includes("overflow-x: hidden"),
    "the block axis of a vertical page must not scroll",
  );
  assert(
    css.includes(
      "html::-webkit-scrollbar, body::-webkit-scrollbar { display: none",
    ),
    "paginated documents must hide publisher and root scrollbars",
  );

  assert(
    reflowableScrollAxis("vertical-rl", "reflowable-paginated") === "vertical",
    "vertical pagination advances along physical Y",
  );
  assert(
    reflowableScrollAxis("horizontal-tb", "reflowable-paginated") ===
      "horizontal",
    "horizontal pagination advances along physical X",
  );
  assert(
    reflowableScrollAxis("vertical-rl", "reflowable-scroll") === "horizontal",
  ...
  const extent = reflowablePageWidth(
    margined,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
    "vertical-rl",
  );
  const gap = reflowablePageGap(
    margined,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
    "vertical-rl",
  );
  assert(
    extent === 552 && gap === 48,
    "a 4% page margin must take 24px off each edge of a 600px page",
  );
  assert(
    extent + gap === 600,
    "a vertical page advance must equal exactly one viewport",
  );
  ...
  assert(
    css.includes("padding-block: 1em"),
...
// first page turn into the chapter land on an empty page.
{
  const rtlPublication: Publication = {
    ...
    rendition: { ...publication.rendition, spread: "auto" },
    spine: [
      { ...item, rendition: { layout: "reflowable", pageSpread: "left" } },
    ],
  };
  ...
  assert(
    placed.spread.execution === "intra-document" &&
      placed.spread.mode === "double",
    "fixture must produce the two-up plan the leading blank applies to",
  );
  assert(
    placed.spread.placement === "left",
    "authored page-spread placement must survive planning",
  );

  const vertical = buildReflowableLayoutCss(
    placed,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
    "vertical-rl",
  );
  assert(
    !vertical.includes("break-after: column"),
    "vertical pagination must not spend its first page on a blank alignment column",
  );

  const horizontal = buildReflowableLayoutCss(
    placed,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
    "horizontal-tb",
  );
  assert(
    horizontal.includes("break-after: column"),
...
// 5. Scrolled documents explicitly remove multicol pagination.
{
  ...
  assert(
    ...
    "scrolled-doc must select the scroll renderer",
  );
  const css = buildReflowableLayoutCss(
    plan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
  ...
  assert(
    css.includes("column-width: auto"),
    "scroll renderer must disable forced column width",
  );
  assert(
    css.includes("overflow: auto"),
    "scroll renderer must expose document overflow",
  );
  assert(
    css.includes("scrollbar-color: rgba(96, 98, 94, 0.58) transparent"),
    "scroll renderer must use the reader scrollbar treatment",
  );
}
...
// declarations into the publication document.
{
  ...
  assert(
    css.includes('font-family: "Reader\\"; color: red"'),
    "font family must remain one escaped CSS string",
  );
  assert(
    !css.includes('font-family: "Reader"; color: red'),
    "authored user value must not break out of the declaration",
  );
}
...
// the physical page advance used by navigation.
{
  ...
  assert(
    pageWidth === 640 && inset === 80,
    "10% margins should leave a 640px text column inside an 800px page",
  );
  assert(
    pageWidth + pageGap === 832,
    "margin space must preserve the original page width plus renderer gutter",
  );
  const css = buildReflowableLayoutCss(
    plan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
  ...
  assert(
    css.includes("inset-inline-start: 80px") &&
      css.includes("column-gap: 192px"),
    "layout CSS should center every text column inside its physical page slot",
  );
}
...
// ordinary reflowable path.
{
  ...
  assert(
    plan.contentPage === pureImagePage,
    "renderer plan must retain the preflight page profile",
  );
  const css = buildReflowableLayoutCss(
    plan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
  ...
  assert(
    css.includes("body img {") && css.includes("position: fixed !important"),
    "a pure image page must escape indefinite publisher wrapper geometry",
  );
  assert(
    css.includes("max-width: 1600px !important") &&
      css.includes("max-height: 650px !important"),
  ...
  assert(
    !css.includes("column-fill: auto"),
    "a pure image page must not enter prose fragmentation",
  );
  ...
  const unassistedCss = buildReflowableLayoutCss(
    unassistedPlan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
  );
  assert(
    unassistedCss.includes("column-fill: auto") &&
      !unassistedCss.includes("position: fixed !important"),
  ...
  const captionedCss = buildReflowableLayoutCss(
    captionedPlan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
  );
  assert(
    captionedCss.includes("column-fill: auto") &&
      !captionedCss.includes("position: fixed !important"),
  ...
  const scrolledCss = buildReflowableLayoutCss(
    scrolledPlan,
    DEFAULT_REFLOWABLE_RENDERER_POLICY,
  );
  assert(
    scrolledPlan.renderer === "reflowable-scroll" &&
      !scrolledCss.includes("position: fixed !important"),
...
// candidate separator.
{
  const candidates = parseSrcset(
    "data:image/svg+xml,%3Csvg%3E 1x, images/a.png 2x",
  );
  assert(candidates.length === 2, "data URL srcset must remain one candidate");
  assert(
    candidates[0]?.url === "data:image/svg+xml,%3Csvg%3E",
    "data URL payload comma must be retained",
  );
  assert(
    candidates[0]?.descriptor === "1x" && candidates[1]?.descriptor === "2x",
    "srcset descriptors must be retained",
  );
}
...
// intentionally remote base, while local bases remain container constrained.
{
  ...
  assert(
    local.path === "EPUB/assets/images/cover.png",
    "local document base must resolve relative to the XHTML document",
  );
  ...
  assert(
    remote.remote &&
      remote.href === "https://example.com/book/images/cover.png",
    "remote HTML base must preserve remote URL semantics",
  );
  ...
  const sectionBase = resolvePublicationDocumentBase(
    "EPUB/text/chapter.xhtml",
    "../assets/",
    "sections/",
  );
  assert(
    sectionBase === "../assets/sections/",
    "nested xml:base must retain inherited directory semantics",
  );
  const nested = resolvePublicationDocumentReference(
    "EPUB/text/chapter.xhtml",
    sectionBase,
    "images/figure.png",
  );
  assert(
    nested.path === "EPUB/assets/sections/images/figure.png",
    "descendant resources must resolve through nested xml:base",
  );
  ...
  assert(
    remoteSection === "https://example.com/book/parts/",
    "nested xml:base must preserve an inherited remote base",
  );
}
...
// whole columns and the measured extent carries error from two directions.
{
  ...
  assert(
    oneShortPage.pageCount === 1,
    `a single column measured one pixel taller than its fragmentainer is still one page, got ${oneShortPage.pageCount}`,
  );
  ...
  for (const [pageExtent, pageGap] of [
    [712, 0],
    [560, 40],
    [480, 120],
    [384, 216],
