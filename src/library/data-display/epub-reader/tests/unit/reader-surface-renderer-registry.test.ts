...
import type {
  AnyReaderSurfaceRenderer,
  ReaderSurfaceRendererKind,
} from "../../react/surfaces/model";

function assert(condition: unknown, message: string): asserts condition {
  if (!condition)
    throw new Error(
      `Reader Surface Renderer registry unit test failed: ${message}`,
    );
}
...
const renderer = (
  kind: ReaderSurfaceRendererKind,
  value: string,
): AnyReaderSurfaceRenderer =>
  ({
    kind,
...
assert(
  Object.isFrozen(registry) && Object.isFrozen(registry.renderers),
  "registry output must be immutable",
);
assert(
  registry.resolve("footnote") === registry.renderers[0],
  "configured provider must replace the default for its semantic kind",
);
assert(
  registry.resolve("footnote") !== defaultFootnote,
  "single-provider override must not form a renderer chain",
);
assert(
  registry.resolve("image")?.kind === "image",
  "unreplaced defaults must remain active",
);
assert(
  registry.renderers.map((item) => item.kind).join(",") === "footnote,image",
  "provider order must follow fixed semantic kind order",
);

assertThrows(
  () =>
    createReaderSurfaceRendererRegistry(
      [],
      [renderer("mark", "one"), renderer("mark", "two")],
    ),
...
assertThrows(
  () =>
    createReaderSurfaceRendererRegistry(
      [],
      [{ kind: "panel", render: () => null } as never],
    ),
