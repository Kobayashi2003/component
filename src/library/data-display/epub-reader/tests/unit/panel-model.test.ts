...
assert(tocItemCount(toc) === 3, 'nested TOC entries must be counted');
...
assert(groups.length === 2, 'marks in the same content document must share a group');
assert(groups[0]?.chapter.label === 'Opening' && groups[1]?.marks.length === 2, 'groups must follow reading order');
