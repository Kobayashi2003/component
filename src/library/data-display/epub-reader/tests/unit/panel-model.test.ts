...
import { chapterContext, filterMarks, groupMarksByChapter, groupSearchHitsByChapter, locatorHref, markLocator, markPreview, tocItemCount } from '../../react/panels/panel-model';
...
const previewMark: ReaderMark = {
  ...
  kind: 'highlight',
  range: {
    start: { href: 'text/a.xhtml', spineIndex: 1, locations: { progression: 0.25 }, text: { highlight: 'A durable content preview' } },
    end: { href: 'text/a.xhtml', spineIndex: 1, locations: { progression: 0.3 } },
  },
  color: 'yellow',
  highlight: 'solid',
  createdAt: '',
...
assert(filterMarks([...marks, previewMark], 'highlight').length === 1, 'mark filters should expose one kind without changing source order');
assert(markLocator(previewMark).locations.progression === 0.25, 'mark navigation should use the range start consistently');
assert(markPreview(previewMark) === 'A durable content preview', 'mark previews should fall back to locator text context');
...
assert(searchGroups.length === 2, 'search hits should be grouped by their closest TOC chapter');
assert(searchGroups[1]?.chapter.label === 'Later' && searchGroups[1]?.results[0]?.index === 1, 'search groups must preserve global result indexes and fragment context');
assert(locatorHref(searchHits[1]!.range.start) === 'text/b.xhtml#later', 'locator fragments should participate in navigation grouping');

function searchHit(id: string, href: string, spineIndex: number, fragment?: string): SearchHit {
