...
function boundaryViolation(sourceArea, source, target) {
  if (
    sourceArea === "core" &&
    (isInside(target, roots.react) || isInside(target, roots.showcase))
  ) {
    return "core must not depend on React or showcase modules";
  }
  if (
    sourceArea === "react" &&
    isInside(target, roots.core) &&
    target !== roots.core
  ) {
  ...
  if (
    isInside(source, coreRoots.features) &&
    isInside(target, coreRoots.runtime)
  ) {
    return "reading features must not depend on reader runtime modules";
  }
  if (
    isInside(source, coreRoots.extension) &&
    isInside(target, coreRoots.features)
  ) {
    return "generic extension mechanisms must not depend on concrete reading features";
  }
  if (
    isInside(source, coreRoots.compatibility) &&
