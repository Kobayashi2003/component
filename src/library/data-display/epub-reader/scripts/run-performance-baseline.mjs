...
const maxTotalMs = Math.max(1, Number(process.env.EPUB_PERFORMANCE_MAX_TOTAL_MS ?? 5000));
const maxHeapDeltaMiB = Math.max(1, Number(process.env.EPUB_PERFORMANCE_MAX_HEAP_DELTA_MIB ?? 128));
...
try {
  ...
  for (const test of manifest.cases) {
    ...
    if (!loaded.publication) throw new Error(`Could not load package for ${file}`);
    ...
    rows.push(row);
    console.log(`${row.id}: ${row.totalMs} ms (${row.sizeMiB} MiB, ${row.spineItems} spine items)`);
  }

  const violations = rows.flatMap(row => [
    ...(row.totalMs > maxTotalMs ? [`${row.id}: total ${row.totalMs} ms exceeded ${maxTotalMs} ms`] : []),
    ...(row.heapDeltaMiB > maxHeapDeltaMiB ? [`${row.id}: heap delta ${row.heapDeltaMiB} MiB exceeded ${maxHeapDeltaMiB} MiB`] : []),
  ]);
  const report = {
  ...
  writeFileSync(join(reportDirectory, 'real-world.json'), JSON.stringify(report, null, 2) + '\n');
  console.log(JSON.stringify(report, null, 2));
  if (violations.length) throw new Error(`Performance budget failed with ${violations.length} violation(s).`);
} finally {
  rmSync(out, { recursive: true, force: true });
}
