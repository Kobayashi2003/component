...
const diagrams = [
  ['docs/diagrams/architecture-overview.drawio', 'docs/diagrams/architecture-overview.svg'],
];

const absoluteCandidates = process.platform === 'win32'
  ? [
      process.env.LOCALAPPDATA && resolve(process.env.LOCALAPPDATA, 'Programs/draw.io/draw.io.exe'),
      process.env.ProgramFiles && resolve(process.env.ProgramFiles, 'draw.io/draw.io.exe'),
      process.env['ProgramFiles(x86)'] && resolve(process.env['ProgramFiles(x86)'], 'draw.io/draw.io.exe'),
    ]
...
const candidates = [configuredExecutable, ...absoluteCandidates, ...commandCandidates]
  .filter(Boolean);
