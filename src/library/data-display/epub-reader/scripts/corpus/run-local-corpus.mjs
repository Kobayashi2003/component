...
try {
  ...
  const { runEpubCorpusCase } = require(
    join(out, "core", "validation", "conformance", "corpus.js"),
  );
  ...
  for (const test of manifest.cases) {
    ...
    observations.push({
      ...
      publicationOpened: result.publicationOpened,
      compatibilityStatus: result.compatibilityStatus,
      diagnosticCodes: result.diagnostics.map((d) => d.code),
      failures: result.failures,
    });
    console.log(
      `${result.passed ? "PASS" : "FAIL"} ${test.id} (${result.compatibilityStatus})`,
    );
    if (!result.passed) {
      failed += 1;
  ...
  writeFileSync(
    join(resultDirectory, "local.json"),
    JSON.stringify(
